...
//let sls_sql_delete_FormTime_records =  "select id from slslog where %f > timestamp AND (%f - timestamp) >= %f"

let sls_sql_delete_FormTime_id = "delete from slslog where %f > timestamp AND (%f - timestamp) >= %f"

let sls_sql_delete_All = "delete from slslog"

let SLS_MAX_DB_SAVE_RECORDS = 10000

let timeoutDelete = 172800.0
...
open class DBManager: NSObject {
    ...
    open func insertaaRecords(endpoint: String, project: String, logstore: String, log: String, timestamp: Double,deleteAllClosure: @escaping (()->())) {
        DispatchQueue.global().async {
            self.dbQueue?.inDatabase({ (db) in
                do {
                    try db.executeUpdate(sls_sql_insert_records, values: [endpoint, project, logstore, log, timestamp])
    ...
    open func deleteRecordFromTime() {
        DispatchQueue.global().async {
            self.dbQueue?.inDatabase({ (db) in
                do{
                    let timestamp = Date().timeIntervalSince1970
                    let sql = String(format: sls_sql_delete_FormTime_id, arguments: [timestamp,timestamp,timeoutDelete])
                    try db.executeUpdate(sql, values: nil)
                } catch {
                    print("failed to delete record: \(error.localizedDescription)")
                }
    ...
    open func asyncDeleteAllRecord(deleteAllClosure: @escaping (()->())) {
        DispatchQueue.global().async { [weak self] in
            guard let `self` = self else {return}
            self.dbQueue?.inDatabase({ (db) in
                do {
                 try db.executeUpdate(sls_sql_delete_All, values: nil)
                    deleteAllClosure()
                } catch {
                    print("删除失败")
                    deleteAllClosure()
