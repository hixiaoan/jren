...
extension Notification.Name {
    static let UliveRefresh = Notification.Name("refreshListData")
    static let UliveLevelClick = Notification.Name( "listLevelClick")
    static let ReloadFMDB = Notification.Name( "ReloadFMDB")
    static let ClassRoomExit = Notification.Name( "ClassRoomExit")
    static let ClassRoomRefresh = Notification.Name( "ClassRoomRefresh")
}

func Notification(_ name: Notification.Name) -> Observable<Notification> {
...
open class CacheCheckManager: NSObject {
    ...
    open func startMonitor() {
        ...
        gcdTimer?.setEventHandler { [weak self] in // `[weak self]` only needed if you reference `self` in this closure and you want to prevent strong reference cycle
            ...
            if let canPost = DefaultsTool.getValue(.logEnable), canPost == true {
                DBManager.defaultManager().deleteRecordFromTime()
                self.postLogsFromDB()
            }
        }
        gcdTimer?.resume()
    ...
    private func postLogsFromDB() {
        ...
        if let enable = DefaultsTool.getValue(.logEnable), enable {
            if shouldPostViaWiFi || shouldPost {
                      ...
                      NotificationCenter.default.post(name: .ReloadFMDB, object: nil)
